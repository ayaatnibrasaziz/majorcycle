...
|---|---|---|---|
| **Pure** — no browser, no network | nothing | A rule you can state as a function, **or a rule about the source itself** | `entitlement.spec.ts`, `export-parity.spec.ts`, `public-chrome.spec.ts`, `auth-contracts.spec.ts`, `form-errors.spec.ts`, `screener-filters.spec.ts`, `log-redaction.spec.ts` |
