...
- [x] **Ready-made baskets** *(new — solves the blank canvas)* — Index (S&P 500 / ASX 200 / TSX 60) + Top-by-market-cap + compact "By sector ▾" + **"By industry ▾"** (industries grouped under their sector via `<optgroup>`) + "Magnificent Seven", from the light universe index (`baskets.ts`, registry — future "My Watchlist" drops in here). **The three index baskets resolve to the *actual constituents we cover* (membership ∩ universe)** *(2026-06-23)* — a committed `web/lib/index-membership.ts` (generated by `analytics/universe/generate_index_membership.py` from the seed `analytics/universe/*.csv`) replaces the old `byMarket()` "every equity in the market" behaviour, so the labels are accurate (S&P 500 → 477 covered, ASX 200 → 175, S&P/TSX 60 → 67) and Request-a-Ticker additions no longer leak into them. Refresh on a rebalance = update the CSV, re-run the generator, commit. *(Automating this refresh — incl. fixing the stale seed lists — is the next session.)*
