...
> | **GA-3** | **An intermittent 404 on Stock Detail, seen locally.** `entitlement-routes.spec.ts` → *"a FREE viewer's Stock Detail HTML contains no scored value anywhere"* failed ~1 run in 3 against `/stocks/us/<TICKER>`, and the captured page was **"Page not found"** — i.e. `notFound()`, i.e. `fetchStockDetail` returned `null`. ⚠️ **Proved pre-existing, not caused by the Layer G polish**: stashing every change and re-running passed, then restoring them gave 1 fail / 2 passes, so the control's pass was luck and the defect is timing, not code. ⚠️ **This is the exact shape CLAUDE.md 11e is about** — `null` must mean "not in our universe" and read failures must throw so the route answers 503. An intermittent 404 says either a row really is intermittently absent, or some read path still folds a soft failure into `null` and 11e missed it | 🟡 **STILL OPEN — narrowed and measured 2026-08-18, nothing relaxed.** (i) **Hypothesis (b) is ruled out:** `readStockRow` throws `StockReadError` on a PostgREST error and `loadPriceBars` throws on both the count and each page, so a failure cannot silently become `null`. 11e's fix is intact. (ii) **Caught in the act with a NEW symptom** — `page.goto` on `/stocks/us/AAPL` **timed out at 45s** rather than 404ing, which reframes it as *slowness*, not absence. (iii) **The data layer is not the cause:** reads total ~2s. Four earlier runs of the same test took 17.4 / 20.7 / 21.0 / 21.3s — steady, with the FIRST fastest, so not a cold-compile spike; ~18 of those 20s is not data. ⚠️ **Side-finding, measured 4×:** the `get_price_bars_json` "fast path" is **~50% SLOWER** than the paginated fallback it replaced (1.7s vs 1.15s for AAPL's 11,510 bars) — the 12 round-trips it avoids run in parallel. **Not changed:** a real trade-off, and not decidable from Australia against a us-east-1 DB. **NEXT:** time `page.goto` alone over ~20 runs and time `/api/cycle` separately, to split "uniformly slow" from "occasionally stalls". Do not raise the timeout, loosen the assertion, or switch `load` → `domcontentloaded` until that says which |
