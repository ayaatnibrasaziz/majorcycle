...
> ⚠️ **AND THIS CORRECTION WAS ITSELF LOW, re-measured 2026-09-14.** *"46 on the screener"* was
> the **upsell card**: this pass signed in with the free shared account, so it never loaded the
> screener at all. On an **entitled** account `/run` is **188px** and `/results` is **158px**.
> The range is **34–188px**. A guard's account is part of its scope, exactly as its route list
> is — see `docs/layer-g-coverage-map.md` § *The Layer H delta*.
>
...
>
> ✅ **P9 · session 2 — the platforms' go-live checklists, DONE 2026-09-07.** The eleven ⬜
> boxes left on 2026-08-31 were answered against the **live accounts**, never the repo — which
> is the whole point of the pass, and the exact thing session 1 got wrong. **Nine closed with
> evidence:** email confirmations are on (`mailer_autoconfirm: false`), only `email` and
> `google` are enabled with anonymous sign-ins off (decision #22, nothing left switched on
> from an experiment), the Security Advisor is still INFO-only, the free-plan inactivity pause
> cannot bite (two crons write nightly), a rollback target is `READY` and promotable, previews
> are still SSO-gated, apex → www is a **308**, and a real post-deploy verification of
> production passed — every public page 200, the gated ticker page 307, all five security
> headers, CSP **enforcing**.
>
> 🟠 **One real defect, 5A-163, found and fixed.** Stripe's checklist asks that logs hold no
> card data or PII and nobody had ever read a log line for it. Card data is a non-question
> (Checkout is hosted). But `lib/email/send.ts` logged the **referral subject**, which carries
> a person's own name, and it and `contact/actions.ts` both logged **Resend's raw error body**,
> which describes a request containing a real address. ⚠️ Not theoretical: Vercel's production
> error table shows that exact line firing from `/account` on 2026-09-01. Fixed by **redacting
> rather than silencing** — the body is the only thing that says why a customer's email did
> not arrive. Guarded two ways, and the source half was proven red on purpose.
>
> **Seven left open, each with its reason named.** Three the instruments here genuinely cannot
> see (Supabase OTP expiry — `get_advisors` returns *database* lints only, so its silence is
> not evidence; SSL enforcement; the Vercel firewall). Four are owner decisions rather than
> measurements: 2FA on both platforms, the Stripe restricted-business check, exercising the
> rollback (it changes what production serves), and Speed Insights.
>
> ✅ **Session 3, same day — all three "cannot see it" rows read in the dashboards.** The
> Supabase email OTP expires in **3,600 seconds**, exactly the 1 hour the checklist asks for.
> **Enforce-SSL is OFF** and should go on — a repo-wide grep finds no direct Postgres connection
> at all, so nothing can break. **Network restrictions are DECLINED rather than undone**: both
> our callers (Vercel, GitHub Actions) have dynamic IPs, so an allow-list would break the site
> and the nightly refresh. The **Vercel firewall is active** and in the past day allowed 598
> requests and denied **1,100** — four Google Cloud IPs scanning a site that has not launched,
> all absorbed by the platform, nothing to do. **2FA is Inactive on both Vercel and Supabase**;
> Stripe could not be read, because its dashboard is signed out and signing in needs the owner's
> own credentials.
>
> ✅ **Then the owner authorised the changes, and all three landed the same day.** **Enforce-SSL
> is ON** — applied with the test suite stopped, because the dashboard warns of a database
> restart the approval had not covered, and re-verified by running the whole suite against the
> enforced database afterwards rather than assuming it was safe. **2FA is now ACTIVE on Vercel
> and Supabase**, enrolled by the owner. And **Stripe's was active all along** — an authenticator
> app added 10 July 2026 — which **closes `5A-003`** and vindicates writing that finding as
> *confirm* rather than asserting: the owner's recollection said no, the audit log said yes, and
> the log was right.
>
> ⚠️ **What remains is RECOVERY, not authentication.** Supabase's own page now warns that losing
> the single authenticator *"will permanently lock you out"*, and Stripe holds no backup email and
> no phone. For a solo operator there is no colleague to be let back in by. The Stripe backup
> email was added the same day; a second authenticator remains the thinnest point.
>
> ✅ **Then the owner said "do the other three", and P9 closed to ONE open row.** **Rollback
> exercised in both directions** and measured on the wire with a build fingerprint, because
> "the dashboard says Production" is a claim about Vercel's UI rather than about what a reader
> is served — site up throughout, apex still 308, gated routes still 307. Two facts only doing
> it could surface: a rollback **also reverts cron jobs** (checked first; `vercel.json` is
> identical across both deployments, so nothing was at risk — that will not always be true), and
> **on Hobby the button reaches exactly ONE deployment back**, which is a second argument for
> Hobby → Pro that nobody had made. **Bot protection is on in LOG mode**, deliberately: Challenge
> blocks "non-browser sources" and **Stripe's webhook is one**, so a challenged webhook would
> fail silently while payments looked fine. **Speed Insights is installed** — and checking it
> before trusting it is what saved it: the package carries a **second, external script host**
> our enforcing CSP does not allow, so in production it would have been silently blocked and
> collected nothing, presenting as an empty dashboard rather than a broken one. Production uses
> the same-origin path; the external host is dev-only, is now allowed in the dev policy alone,
> and **the shipped `script-src` is byte-identical to before**. Render modes re-checked (9
> prerendered, 6 dynamic, unchanged) because a client component in the ROOT layout is what made
> the whole site dynamic once before. **Only the Stripe restricted-business check remains.**
>
> ❌ **And a correction this repo owed itself: Supabase is on the PRO plan (US$25/mo), not the
> free tier.** Four documents had said free tier since Phase 1 — CLAUDE.md twice,
> `architecture.md` twice, `glossary.md` and `roadmap.md` — and nobody had looked at the billing
> page. ⚠️ **Spend cap is ON**, so past the included quota the project goes **read-only** rather
> than billing more: under a spike the product stops, it does not overcharge. Probably the right
> trade pre-revenue, now written down as a choice rather than a launch-day discovery.
...
> ✅ **COMPLETE AND LIVE 2026-09-25** — H1–H5 and H6a merged (PRs #101–#115); H6 settled (text size: no change; lawyer: replaced by the legal re-audit; beta: owner-run). Launch now waits on the owner's items only — see the top of this file.
>
...
>
> **H3, H4, H5 merged (PRs #106, #107, #108).** ✅ **H6a merged and live 2026-09-25 (PR #113), with #114 and #115 — LAYER H'S BUILD IS COMPLETE.** H6 (owner items) is settled below. *(What follows is the H1 build note as written.)*
>
...
> contradicts non-negotiable #1. **A "not done" row is a claim about the last session's
> measurement, exactly as a "done" row is a claim about its intent** (11aj).

**Already fixed — removed from this list, each re-checked on 2026-09-14:**

| Was on the list | Actually |
|---|---|
| Public site has no navigation 375–900px | **FIXED** (5A-156). `MenuButton` is `min-[900px]:hidden`, so the menu exists at every width below 900 |
| `[data-legacy-contrast]` on the screener chips | **GONE** — the guards now fail if the marker returns |
| Direction colours used as text (the remaining six) | **CLOSED** 2026-08-22 when the owner brought the contrast sweep forward |
| Accessibility audit (axe-core) | **Substantially done** in G + 5a, including the paid account and the five `experimental` rules |
| CSP verification ("→ Layer H") | **DONE in G7**, enforcing since 2026-08-23 |
| 320px header overflow | **FIXED** (5A-154/159) — all eight public pages measure `scrollX = 0` at 375 **and** 320 |

**⛔ Deleted: "Final design review against reference HTML."** CLAUDE.md #1 demoted that file from
contract to mock-up on 2026-08-22 — *"the live site is decided with the owner, not derived from
the file"*. A review *against* it is the activity that rule exists to stop. **5b owns that slot.**
...
- [x] **Below 768px the sidebar becomes a drawer**; the header loses its 220px offset and gains
  ...
  - **The overflow is 34–188px and varies by page — not "~130px".** Measured on the production
    build at 375px, on a **paying** account and a free one:

    | Page | Free | **Paying** | Clean from (sidebar present) |
    |---|---|---|---|
    | Run Analysis | 46px | **188px** | 620px |
    | Stock Detail | 180px | 180px | **730px** |
    | Results | — | **158px** | 620px |
    | Browse | 131px | 131px | 520px |
    | Account · Request | 55 · 34px | 48 · 34px | 440px |

  - 🔴 **`e2e/app-responsive.spec.ts` has never measured a page a paying customer sees.** It
    signs in with the shared no-subscription account, so the paid sections never render. It
    sweeps 640/768/900/1280 and passes — while the **entitled** ticker page scrolls at 640 and
    720, inside the very band it was written to guard (5A-116). Same family as 14g.
  - ⚠️ **Tablets keep the desktop layout** — the owner asked, and the measurement agreed: every
    real iPad portrait width (744 → 1024) is **zero** on every page with the sidebar in place.
    768 is chosen over the 730px boundary for 38px of margin; the iPad mini (744px) takes the
    drawer rather than rely on 14px of luck.
  - **Proof:** sweep 320 → 1280 in 4px steps, **entitled and free**, assert **≥20px of spare
    room** rather than "no overflow", with the offset restored on one page as a deliberate
    break, plus a control that the sidebar still exists at 1280px.
  - ⚠️ Residual, stated now: at **320px** — below the 375px floor #3 commits to — the screener is
    expected to remain ~23px over. Everything else fits.
...
- [x] Full install, not money-paths-only. ⚠️ Two docs disagreed: the roadmap listed it here, the
  ...
  - **The money paths first.** All six log lines re-verified present on 2026-09-14 at the paths
    below. Each is handled correctly in code and announced only to a `console.error` nobody reads:
    - **`billing sync: DUPLICATE SUBSCRIPTION`** (`lib/billing/sync.ts:150`) — the guard cancels
      the duplicate, but **cancelling does not refund**, so a duplicate already charged needs a
      manual Dashboard refund. **Nobody currently finds out.**
    - `billing sync: could not verify existing subscription` (`sync.ts:135`) — the fail-open path
    - `stripe webhook: no profile for …` (`webhook/route.ts:103, 197, 246`) — someone may have
      paid and not been provisioned
    - `portal: could not create billing portal session` (`portal/route.ts:104`) and checkout's 502
    - `[freeViews] record_free_view failed` (`lib/freeViews.ts:99`) — the anti-scraping fence has
      stopped counting
  - Plus a breadcrumb for `CYCLE_INTERNAL_SECRET` mismatches, which render every Stock Detail
...
- [x] **Whole site in Chromium + Firefox + WebKit, run LOCALLY on demand, deliberately NOT in
  ...
  - ⚠️ **It gets its own command and `pnpm gates` prints it as NOT RUN with the reason**, the way
    `lighthouse` is handled — a suite that runs in no workflow is a gate nobody runs, which is
    audit **F-016** exactly.
  - ✅ **All three engines now run here.** Firefox would not start at all; the cause was this
    machine's sandbox redirecting `%LOCALAPPDATA%\ms-playwright` into the Claude app's private
    container, which breaks Firefox's private side-by-side `mozglue` assembly. Fixed with
    `PLAYWRIGHT_BROWSERS_PATH` pointing outside AppData — see memory
    `reference-claude-sandbox-and-vercel-cli`.
  - 🟢 **Measured clean already:** all eight public pages in all three engines — 200, zero console
...
#### H6 · Owner items — settled 2026-09-25

- [x] ~~Disclaimer copy review by a lawyer~~ — **owner: not doing one.** Replaced by a check of every
      legal page against the running system; four gaps found and fixed (`legal-audit.md`,
      *Re-audit 2026-09-25*). Two questions a page edit cannot settle are recorded there.
- [ ] Beta with 5–10 friendly testers, then fix all P0/P1 from it — 🔵 **owner-run**, not a code item
- [x] The **13px body size / 16px input** decision (5A-155) — **CLOSED, no change** (owner: *"happy
      with what is present in the live site"*). iOS zoom-on-focus is accepted; do not re-propose.

#### H6a · Twelve new Key Metrics rows ✅ — MERGED AND LIVE 2026-09-25 (PR #113)

✅ **MERGED AND LIVE 2026-09-25 (PR #113).** ⚠️ The first production deploy served the OLD stylesheet
  from Vercel's restored build cache — new rows, old colours — and the owner saw it; PR #115 makes
  every production build compile fresh and adds `pnpm check:live-css` (63/63 after). CLAUDE.md 11bl.
  *(Build note as written:)*
...
- [x] **Key Metrics goes from 13 rows to 25, in six groups instead of four.**
  ...
  - ⚠️ **The old row said "three fundamentals we store, score with, and never show", and it was
    wrong twice over.** Only **two** of the three are scored — `forwardPe` feeds nothing, which
    `roadmap.md` already said 600 lines above (11av). And sweeping every field against every
    surface found **nine** numbers a customer cannot see, not three: the known three plus
    **Price/Book, Price/Sales, EV/Revenue, Quick Ratio, Beta and EBITDA Margin** — all 91–99%
    covered, every one better populated than the PEG ratio already shipping.
  - **Owner, 2026-09-14: add all nine, plus Payout Ratio and Short Interest for the peer
    comparison they cannot get in their own sections; yes to the report and the exports.**

    | Group | Rows | New |
    |---|---|---|
    | Valuation | 4 → **8** | Forward P/E, Price/Book, Price/Sales, EV/Revenue |
    | Profitability | 5 → **7** | EBITDA Margin (between Gross and Operating), FCF Margin |
    | Growth | 2 | — |
    | Balance Sheet | 2 → **3** | Quick Ratio |
    | **Shareholder** 🆕 | **2** | Payout Ratio, Share Count change |
    | **Risk** 🆕 | **3** | Beta, Short % of Float, Days to Cover |

  - 🔴 **The Risk rows must carry NO better/worse verdict.** `higherBetter` is a required boolean
    and every comparison is coloured and captioned *"stronger/weaker than the typical peer"* —
    false for all three: a higher beta is more volatile, not weaker, and our own Short Interest
    tooltip says elevated shorting *"can also fuel a short-squeeze rally"*. `higherBetter`
    becomes `boolean | null`. Same principle as `direction-not-rating.spec.ts` (5A-135).
  - ❌ **Dividend Yield is deliberately NOT added.** `DividendHistory` computes its own trailing
    yield rather than showing the stored one; across 60 payers they disagree by >0.25pp on
    **27%** and by **15.21pp** on HLI.AX (6.08% vs 21.29%). Adding it prints two different
    dividend yields on one page — 11c-iii at fifteen points instead of a cent.
  - ⚠️ **Short interest is a US number**: `shortPctOfFloat` is US 99% / CA 19% / **AU 0%**. Not a
    blocker — `MetricsTable` omits a row with no value rather than blanking it — but the Risk
    group is three rows in the largest market and **one row in the owner's home market**.
  - ⚠️ **Three pillar tooltips describe inputs the scoring does not use** — "operating cash
    conversion", "dividend yield", "payout consistency", "operating leverage". Correct them in
    the same pass: we describe our own method on a page carrying a rating (#12/#24).
  - ✅ Caps derived from the live distribution, never invented. ✅ The `ebitdaMargin`
    zero-means-not-reported trap checked, not assumed: **no company stores an exact 0.0** for any
    of the four margins, and the nightly invariant already fails if one appears.
  - ⚠️ `--cat-shareholder` and `--cat-risk` are **new tokens** and must clear `check:tier-palette`,
    including the adjacent-pair rule in two simulated colour blindnesses (11t).

**What Layer H cannot check, stated before it starts:** a real iPhone or Android (no device, and
no emulator reproduces iOS Safari's zoom); that Sentry catches a *real* incident; decision #33's
Lighthouse target (no trustworthy instrument — Speed Insights needs traffic); the paid analysis
on a local machine (`/api/cycle` is a Python function `next start` does not serve, 11v); and
whether any of it *feels* right, which is 5b's whole purpose.
...
| ✅ | All tests passing in CI | **Re-read 2026-08-31** from run `33368520554` on `f1b6d4d` — the **post-merge** run on `main`, so it measures what is deployed rather than what a branch hoped to deploy: Python ✅ · Frontend ✅ · E2E **699 passed** · `pytest` **247**. Grepping the whole log for `failed`/`skipped`/`flaky` returns **zero lines**. *Previously:* 617 / 189 on `bc7672e`, 2026-08-25. No `flaky` and no `skipped` line in either summary — read the whole block, not the last line matching "passed" (CLAUDE.md 11i). ⚠️ This row said **105/105 and pytest 86** on `ab11e18` until 2026-08-25, a reading taken 2026-08-02 and three weeks stale; the suite had grown five-fold underneath it — and it then aged again, to 617/189, before being re-read on 2026-08-31. **A cited SHA does not stop a number ageing; re-citing it does.** *The caveat that used to sit here — that the audit's own commits were local and unpushed, so CI had not seen them — is now spent: Layer G merged as `f1b6d4d` and this is CI's reading of that commit* |
...
| ⬜ | Mobile responsive at 375px width | **Layer H, planned and designed 2026-09-14.** ⚠️ This cell said *"~130px overflow"*; re-measured it is **34–188px and varies by page**, worst on the **entitled** screener (188px) which no test had ever loaded. Root cause confirmed: `Sidebar` is `fixed w-[220px]` with no breakpoint. Public pages are clean at 375 **and** 320. Fix is scoped to phones only — every iPad keeps the desktop layout (`layer-h-plan.md` §11) |
...
| | Criterion | Evidence |
|---|---|---|
| 🟡 | All accounts billing configured | Stripe live with payouts to the linked CBA account; Resend and Cloudflare on free tiers that suit the load; **Supabase is on Pro (US$25/mo)**, corrected 2026-09-07 after three docs had said free tier since Phase 1. *Missing, and it is the **launch blocker**:* **Vercel is still Hobby, whose terms forbid commercial use.** Hobby → Pro is the owner's scheduled call at official launch |
| ✅ | Domain DNS verified, SSL cert active | `https://www.majorcycle.com` on a valid certificate; the apex **308s** to `www` (permanent — changed 2026-08-31 in **Vercel**, not Cloudflare; the redirect was always Vercel's). ⚠️ Vercel flags *"DNS Change Recommended"* on both records — advisory, the current ones keep working, worth doing before launch |
...
✅ Phase 1 Layer F: Static Pages + Subscription  (built + merged PR #72 + live 2026-08-01)
   ...
   ↓
✅ Phase 1 Layer G: SEO + Performance         (MERGED 2026-08-31; 5a complete 2026-09-12. The audit
   ...
   ↓
✅ Phase 1 Layer H: Hardening (Phase 1.5)  — COMPLETE + LIVE 2026-09-25 (PRs #101–#115), docs/layer-h-plan.md
   ...
   ↓
🚀 LAUNCH                                   ← NOW — gated on Vercel Hobby → Pro (Hobby forbids commercial use),
                                              the owner's beta + 5b pass
   ↓
Phase 2: Smart Money UI + earnings calendar + watchlists + alerts + FMP
```

**A through H are built, merged and live** (2026-09-25). No build layer remains before launch; what is left is owner-held (above). *(History:)* **A through F are built, live and audited.** Remaining build was **G and H**. Layer F closed on
