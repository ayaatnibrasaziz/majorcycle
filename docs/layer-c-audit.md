...
### S5 — Beginner help (tooltips / jargon explainers + Key Metrics softening) (2026-06-05) — PR pending
Owner approved (AskUserQuestion): **build the reusable ⓘ InfoTip**, **full sweep across all sections**, **no first-visit hint**. Onboarding modal already satisfies decision #23 — left as-is. Pure web change — **no engine/data/Python touched**.
