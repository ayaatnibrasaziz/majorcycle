...
> week with an archive. ⚠️ **That last decision was re-scoped on 2026-08-25** — see G8 below:
> it is now `/articles`, weekly and human-written but with **no fixed template**, aimed at
> search visibility. Its own section and one permanent page each still stand. **Audience = both, served in LAYERS not averaged** — this
