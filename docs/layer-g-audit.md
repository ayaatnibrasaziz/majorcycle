...
| ✅ | Layer 1 coverage map | built, and the uncovered list ruled on by the owner | map built 2026-08-23 → `docs/layer-g-coverage-map.md`; owner ruled on all 8 the same day and **all 8 acted on** ✅ — five defects found (F-005, F-009, F-011, F-012, and 11a's fifth instance). ⚠️ **This row carried an amber marker until 2026-09-12 while its own text said all eight were closed** — a tick that lagged its evidence by two weeks, which is the marker version of the stale "still outstanding" lists in the 5a sweep. **When you write the evidence, move the marker in the same edit** |
