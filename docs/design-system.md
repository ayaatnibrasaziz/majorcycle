...
- **Choose what to analyse** — ready-made **baskets** lead (`BasketPicker`: index /
  ...
  index constituents we cover* — the intersection of a committed membership list
  (`web/lib/index-membership.ts`, generated by
  `analytics/universe/generate_index_membership.py` from the seed
  `analytics/universe/*.csv`) with the universe — **not** "every equity in that
  market" (which would absorb Request-a-Ticker additions). Refresh on a rebalance =
  update the CSV, re-run the generator, commit.
