...
| Plot gutter | Every drawing sits inside **`Plot`** (chartPrimitives): an outer box carrying **`AXIS_GUTTER_PX` = 44px** of left padding, and an inner box that is the plot. `rx()` therefore spans the whole inner box and the y-axis labels live outside it, right-anchored, `AXIS_LABEL_GAP_PX` = 8px clear. ⚠️ The gutter was **15 viewBox units** until 2026-08-21 — 15% of whatever the panel measured — which is right at 375px and a wasteland at 1280px: **172px of empty margin holding a 29px number**, so every chart began a sixth of the way in from its own card. A gutter exists to fit a label and a label does not get wider when the screen does; size it in the units the label is in. ⚠️ **Two boxes, not one:** an absolutely-positioned child lays out against its ancestor's *padding* box, so padding alone leaves `inset-0` spanning the gutter. ⚠️ **Every overlay must be inside the same `Plot` as the drawing it annotates** — the dividend figure's shared time axis is a sibling of its two panels, so when they moved inside the gutter it stayed at the card edge and each year marker sat 44px from the data it labels, still rendering perfectly (11c-iv again). `learn.spec.ts` has no guard for this; a probe comparing every overlay's offset parent with its plot box does, and it is in the scratchpad, not the repo |
...
| Labels on a plot | **Nothing is ever drawn behind a label.** A halo — the panel's ground painted behind the text — was built and rejected on sight: it interrupts the very curve the figure is about, so a chart with a label over a line looks *broken* rather than crowded, and on `dip-correction-crash` it erased a length of the dashed rule the figure exists to explain. So does a rule that computes which side of a point has free space: it worked, went stale-prone, and swapped one collision for another. **The fix belongs upstream** — shape the paths so troughs are separated (see `indexGeometry.ts`), and give a narrow screen more plot HEIGHT, since 12px is the reading floor and the type cannot give. If a label has nowhere to go, the figure is too busy; that is information, not a styling problem |
