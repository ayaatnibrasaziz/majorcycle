...
**Content Security Policy (CSP)** — A header that tells the browser which code the page is allowed to run and where it may load things from. The attack it closes is somebody else's script executing inside our document: on our domain, behind our padlock, holding a customer's session, able to read what is on screen and what is typed. React escapes text by default and this repo injects no raw HTML, so we are in decent shape by habit — **a CSP is the lock rather than the habit.** Ours was `Report-Only` (reports, blocks nothing) from F0.5 until **2026-08-23**, when it became enforcing. ⚠️ A Report-Only policy that is *wrong* is not harmless: ours was missing `accounts.google.com` under `style-src`, which would have broken the Google sign-in button the day anyone flipped it. See [[Nonce]] and `architecture.md` §7.

**Nonce (CSP)** — A random token minted fresh on every page view, stamped by our server onto our own `<script>` tags and named in that response's CSP; the browser then runs a script only if it carries that token. **Nobody outside is granted anything** — a visitor cannot obtain it, because their input arrives after the stamp exists and the stamp is different on every request. It is the strongest form of `script-src`, and the site uses it on every route that renders per request: sign-in, sign-up, pricing, password reset and the whole signed-in product. ⚠️ It cannot be used on a **prerendered** page (`/`, `/contact`, `/disclaimer`, `/learn`, `/learn/[slug]`, `/privacy`, `/terms`), whose HTML was written once at build time and carries no token — sending one anyway refuses every script and leaves the page rendering and then doing nothing. Those seven carry `'unsafe-inline'` instead, which is safe there precisely because they hold no session, no personal data and no form that matters. `usesNonce()` in `lib/csp.ts` is the single rule; `pnpm check:render-modes` asserts the two sets can never overlap.
...
**Learn library** — The public explainers at `/learn` and `/learn/[slug]`, written for a **newcomer** who arrived from a search engine and has never heard of us (owner decision, G2). Grouped by topic rather than dated, because an explainer does not expire. Not to be confused with **articles** (`/articles`), which are the opposite — dated, about what just happened or what we just measured, and in their own section. ⚠️ **Not weekly:** that cadence was dropped by the owner on 2026-08-26, along with the assumption that an article is one genre — the section carries measurements, market commentary and how-to pieces. It was called the "weekly market note" until 2026-08-25. Every article exists in exactly one place, the registry in `web/lib/learn.ts`; see `data-contracts.md` §7b.
...
**Neutral** — Rating tier 3 (score 50-64). Mixed signal. Same label as the original UI. Colour: `--c-tier-3` = **`#9C5B01`, a deep gold since 2026-09-02** — the middle lamp of a traffic light, by owner decision (*"going from green, grey and red doesn't make sense"*). It was gold originally (`#D4A017`), grey from 2026-08-22 to 2026-09-02 because the original measured **2.38:1** under white numerals — a compliance failure on every ranked row and in every downloaded workbook — and gold again once a value was found that clears both floors (5.37 under white, 4.86 as text) instead of neither. ⚠️ The trade, measured and accepted: gold sits 5.7 from Constructive and 5.3 from Cautious for a red-green colour-blind reader where the grey scored 22.0 and 24.6. Not a 1.4.1 failure — every chip carries its score and every badge its word. See `docs/design-system.md` and the comments in `globals.css`.
...
**Open Graph** — The tags that decide what a link looks like when it's pasted into WhatsApp, LinkedIn, Slack or a message: title, description, address, and an image. Set by `pageMetadata()` in `web/lib/seo.ts` on every public page and every Learn article. ⚠️ **This entry said "we ship no `og:image` yet" until 2026-08-22, and had been wrong since 2026-08-08.** One sitewide 1200×630 card (`app/opengraph-image.png`, built by `pnpm build:og-image`) shipped in G2, and `twitter:card` is now `summary_large_image` — which is honest only *because* the image exists. Per-**stock** cards remain forbidden: they are public and cached for everyone, so one carrying a rating would publish paid output on a CDN. `e2e/seo.spec.ts` asserts each indexable page declares **exactly one** `og:image`.
...
**Prerendered (static) route** — A page Next builds to an HTML file at deploy time, marked `○` (or `●` when generated from `generateStaticParams`) in the build output. It is the same bytes for every visitor, served from the edge, and — the reason it matters here — **its full route is prefetched**, so a click costs ~77ms instead of a server round trip. The opposite is a **dynamic route** (`ƒ`), rendered per request, whose prefetch Next *skips* entirely. Which of ours are which is not a per-page choice: one session read anywhere in the shared tree turns the whole site dynamic (CLAUDE.md 11s). Asserted by `pnpm check:render-modes`, which reads the build OUTPUT rather than the source. See `architecture.md` §7.2c.
...
**Soft 404** — A page that says "not found" while answering HTTP **200**. Search engines treat it far more harshly than an honest 404, because they must infer the meaning from the words rather than read it from the status. Ours were sitewide until 2026-08-18: a root `loading.tsx` flushed the response shell before `notFound()` could set the status, so every missing page returned 200. Fixed by deleting that file; guarded by an assertion in `e2e/learn.spec.ts` that carries a "a real article still answers 200" control. See `architecture.md` §7.2.
