...
**Security posture & planned hardening (F0.5 audit — 2026-07).** A full code + platform (Supabase/Vercel/Resend/Cloudflare MCP) audit ran during F0 verification; full findings + fix plan in `plan-mode-auth-virtual-ladybug.md`. **Confirmed strong:** the RLS lockdown above; parameterized queries + stripped LIKE metacharacters (no SQL-injection surface); no service-role key in the client bundle (only `NEXT_PUBLIC_*` are inlined); Python `/api/analyze` + `/api/search` confirmed gated by middleware in prod (unauth → `307 /login`), `/api/cycle` public-by-design leaks no user data; Resend 0 send-errors, Vercel 0 runtime errors. **To fix next (F0.5, before subscriptions):** (A) the password-recovery link mints a *full* session — must be confined to the update-password page; (E, HIGH) the `profiles` UPDATE grant lets a user self-set `subscription_status`/`stripe_*` from the browser (latent billing bypass) — needs column-level GRANTs; (F) add security headers (only HSTS is set today); (J) enable Supabase leaked-password protection; (O) tighten DMARC `p=none → quarantine`. Perf advisors: wrap RLS `auth.uid()` in `(select …)` (L) + covering indexes on 3 FKs (M).
