...
/**
 * The three branded billing-lifecycle emails (F3 Step 8), all fired from the
 * Stripe webhook (`web/app/api/stripe/webhook/route.ts`):
 *   1. trial ending    — ~3 days before the first charge (customer.subscription.trial_will_end)
 *   2. payment failed   — first renewal failure (invoice.payment_failed)
 *   3. payment recovered — a failed payment succeeded (invoice.payment_succeeded)
 *
 ...
 * phrasing ("in a few days") per docs/coding-standards.md §16 — never a device date.
...
/** Email #1 — trial ending reminder, ~3 days before the first charge. */
export async function sendTrialEndingEmail(opts: {
  ...
  const bodyHtml = [
    ...
    p(
      `Your MajorCycle free trial ends in a few days. After it ends, your subscription will ` +
  ...
  const text =
    ...
    `Your MajorCycle free trial ends in a few days. After it ends, your subscription will continue ` +
  ...
  return sendBrandEmail({
    ...
    bodyHtml,
    preheader: 'Your free trial ends in a few days — after that your subscription begins.',
    text,
    idempotencyKey: opts.idempotencyKey,
  });
}

/** Email #2 — first failed renewal payment; nudge to update the card within the grace window. */
...
/** Email #3 — a previously failed payment has now succeeded; all good. */
