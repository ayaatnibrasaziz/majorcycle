...
⚠️ **2026-09-26 — the screener reads a STORED package, and the ticker page no longer carries the whole history.** Measured that morning: screens of 63 s / 86 s / 66 s (short / medium / long), and a ticker page that took 10.1 s cold. The cause of the first was the disk: building a package from `price_bars` cost **329 ms warm and 4,142 ms cold per 25 tickers**, because the covering index is 796 MB against 224 MB of `shared_buffers`. Three changes, none paid:
- **`price_bars_cycle_pack`** holds each ticker's `get_cycle_bars_bin` bytes (185 MB for 881 tickers — small enough to stay in memory). A statement **trigger** on `price_bars` bumps a per-ticker `version` on every insert/update/delete, so no write path can forget; `get_cycle_bars_packed` returns the stored bytes only when they were built from the current version and otherwise builds live, storing the result only if no write landed meanwhile. `rebuild_stale_cycle_packs` (pg_cron, every 20 min) keeps them fresh. Verified after the backfill: **881 of 881 byte-identical** to a live build, with a control proving the comparison flags a different ticker. Raw prices in a second layout — never a rating (#15). Migration `20260926000000_price_bars_cycle_pack.sql`.
- **One fundamentals read per chunk** (`_load_fundamentals_batch`, `.in_()`), not one per ticker. A failed batch returns **None** (read each ticker) and never `{}` (which would call every ticker a non-stock — 11e); `analytics/tests/test_analyze_fundamentals_batch.py`.
- **The ticker page embeds the last `RECENT_BARS` (520) bars** — every chart's default one-year view — and fetches the full history once from **`/api/bars`** (`lib/priceHistory.ts`, `components/stocks/PriceHistory.tsx`). The drawdown chart, which runs over the whole history, waits for it rather than drawing a partial picture. The route is behind sign-in, applies the free tier's daily fence exactly as the page does, and is `private` always: kept a day only when its `v` matches `barsVersion()` — a fingerprint of every price, so a dividend re-adjustment changes the URL. The packing is exact to the bit and a column-length mismatch throws (`e2e/price-history.spec.ts`). Server-rendered sections (Technical Levels, Thesis, Analyst Target) still receive the full series. Measured after deploy, from Australia: `/api/bars` is ~2.3 s on a first visit (≈1.2 s server, ≈1.1 s download of ~370 KB) and **23 ms on a repeat** (browser cache, `transferSize` 0). A `preload` hint is sent, but because it is emitted after the shell has streamed React delivers it in the RSC stream, so it starts only slightly early — see the comment in `page.tsx`.

**Measured on production, 2026-09-26 (before → after).** Screener, 760 stocks: short 63 s → **22.1 s**, medium 86 s → **30.8 s**, long 66 s → **22.9 s** (Long's summary — HUB 89/100, 349 of 751 Constructive or better — identical before and after). Stock page, whole HTML: MSFT warm 3.2 s → **1.1 s**, KO 3.1 s → **0.9 s**, CBA 1.5 s → **1.2 s**; stocks not opened that day ~10 s → **1.4–3.2 s**; page 1,842 KB → **332 KB**. `/request`: its list now arrives inside the page (~0.6 s) instead of a second request after it (~1.4 s). ⚠️ **Never measure while CI runs**: ten E2E shards share this one database, and the same page read 17–40 s during the post-merge run on `main`.
