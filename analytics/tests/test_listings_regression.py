...
CASES = [
    ...
    (1810, 1999, False, "9.5% down is two sources disagreeing, not a collapsed market"),
...
def test_the_real_numbers_from_the_incident() -> None:
    ...
    On 2026-08-25 the ASX source returned 0 against ~1,999 stored AU symbols, and
    the replacement returned 1,810 against the same 1,999. The rule has to separate
    those two, and by a comfortable margin rather than a hair — 1,810 is 90.5% of
    the menu, nowhere near the bound.

    ⚠️ The replacement's count MOVES: 1,841 one hour and 1,810 the next, measured
    from two machines, so it is the source changing rather than a location
    difference. Both figures are asserted, because a rule that only survives one
    snapshot of a moving source is not a rule.
    """
    assert is_regression(0, 1999) is True
    assert is_regression(1810, 1999) is False
    assert is_regression(1841, 1999) is False


# (missing, existing_active, safe_to_deactivate, why)
DEACTIVATION_CASES = [
    # ── must refuse ──────────────────────────────────────────────────────────
    (189, 1999, False, "the measured changeover: 9.5% of the ASX is source disagreement, not delistings"),
    (158, 1999, False, "the same changeover measured an hour earlier — the source moves"),
    (1999, 1999, False, "retiring the entire market"),
    (100, 1999, False, "5% in one night is still two sources disagreeing"),
    # ── must allow ───────────────────────────────────────────────────────────
    (0, 1999, True, "nothing to retire"),
    (3, 1999, True, "three delistings in a week is exactly what the sweep is for"),
    (39, 1999, True, "just under 2% — the sweep must not stop working for real churn"),
...
def test_the_two_rules_are_independent() -> None:
    ...
    rather than one. 1,810 symbols against 1,999 stored is nowhere near a
    regression — the market did not collapse — and yet the ~190 it omits include
    ...
    those two.
    """
    assert is_regression(1810, 1999) is False, "the pull itself is healthy"
    assert is_safe_to_deactivate(189, 1999) is False, "but its omissions must not delist anyone"
