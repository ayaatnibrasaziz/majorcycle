...
**Runtime:** the universe is **867 tickers at 2026-08-04** (863 equities + 4 benchmark indices), split **616 US+CA / 251 AU**, so each run is proportionally shorter than the ~20–30 min the combined run took. **The universe auto-expands — re-read `select count(*) from stocks` rather than trusting this number.** (Both keep `timeout-minutes: 60`; halving the work doesn't justify tightening a safety margin.) On a night when many earnings have just passed the runtime extends proportionally — each enriched fetch takes ~30s per ticker vs ~2s for price-only.
