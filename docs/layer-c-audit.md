...
| 19 | OwnershipStructure | Sentiment | S6 | 🔧 | **S6 audited.** Donut % calc verified vs DB: float = 100−inst−insider (SHOP 100−73.79−0.22 = 25.99 ✓); holders `pct_out` is a fraction → ×100 (BlackRock 0.0779→7.79% ✓); shares ÷1e6 → "M" matches header ✓. 🔧 per-row tips (Institutions/Insiders/Public Float) → InfoTip; donut `aria-label`. 🔧 holders table scrolls within its card at ≤600px (`overflow-x:auto` + `min-width:0`, scoped to mobile — desktop unchanged) so it no longer pushes the page at 375px. **Data note:** DB stores snake_case (`institution_ownership_pct`…); bridged to camelCase by `toCamel` in stocks.ts — read the snake keys when verifying via SQL. |
...
### S6 — Sentiment component audits (2026-06-05) — ✅ built + verified on local dev, awaiting owner merge confirm (branch `feat/s6-sentiment-audit`, commit `7e125ed`)
