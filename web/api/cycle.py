...
class handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        try:
            ...
            preset = (query.get("preset", ["medium"])[0] or "medium").lower()

            if not ticker:
                self._json(400, {"error": "missing required query param: ticker"})
                return
            if preset not in PRESETS:
                self._json(
                    400,
                    {"error": f"unknown preset '{preset}' (must be one of: {sorted(PRESETS)})"},
                )
                return
            ...
            if row is None:
                self._json(404, {"error": f"ticker '{ticker}' not in universe"})
                return

            df = _load_price_bars(sb, ticker)
            if df is None or df.empty:
                self._json(404, {"error": f"no price history for ticker '{ticker}'"})
                return
            ...
            if analysis is None:
                self._json(
                    500,
                    {"error": f"analysis failed for '{ticker}' — insufficient price history"},
                )
                return

            self._json(200, _serialise_analysis(analysis), cache_for_seconds=3600)
