...
| Figure labels | Every piece of text inside a figure's drawing is compared with every other, on every article, at six widths — `learn.spec.ts`, "no two labels overlap, in any figure". ⚠️ It replaced three narrower checks (marker-vs-marker on the analyst figure, tick-vs-tick on two others) that were each blind to the defect that was actually shipping: "Lowest $82" sat on the "$80" **axis tick**, 12px of overlap at every width including 1280, for the life of the figure. A guard scoped to one KIND of label is as narrow as one scoped to one page. Pinned labels use `PinnedLabel` (chartPrimitives), which picks its anchor from the position so a label near the right edge cannot hang off the panel |
| Axis labels | Anchored to the **axis**, `AXIS_LABEL_GAP_PX` = 8px, in `AxisLabels` / `XTickRow` (chartPrimitives). ⚠️ They were `left: 0` until 2026-08-21, which pins a label's *start* and lets its end land wherever the text runs out — so the distance to the axis was a side effect of how many characters the number had. Measured across the library: **57px on nine figures against 12px on two**, i.e. both too far and inconsistently far, and the same figure would have shifted again the day a tick went from `-40%` to `-100%`. Right-anchoring makes the gap the specified thing and the label width the varying one, which is the way round a reader perceives it. The x-tick row hangs off the **floor line**, not the bottom of the box — two figures used `top-full` on a 16:9 panel whose axis sits at 86–88% of it, leaving the year markers **38–40px** adrift. ⚠️ The offset is a **margin, never padding**: padding keeps the gap inside the box, so the element's own rect still ends flush against the plot and every instrument that asks the DOM where a label is reads a collision that is 8px of empty space |
| On-plot labels | Anything drawn inside a plot wears **`PLOT_LABEL_HALO`** — the panel's own ground behind the text, so a curve passing behind a label is interrupted rather than drawn through it. ⚠️ Added 2026-08-21 after a company's recovery ran straight through "Company C −30%" at every width, reading as a deliberate crossing rather than a defect. **The first fix was cleverer and worse**: a rule computing which side of each point had clear space. It worked, and on a phone it put the index label on top of the index line, where the halo punched a visible gap and the chart looked broken. What actually holds is the halo plus **more plot HEIGHT on narrow screens** — the crowding was vertical (four 22px labels in a 211px box) and 12px is the reading floor, so the type cannot give. A cleverer rule was solving a problem that more room did not have |
